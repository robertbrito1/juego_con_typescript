...
export function curar() {
    let partida = cargarSesion() as datos; // Aseguramos que el tipo es datos, aunque cargarSesion ya lo devuelve como datos | null, aquí asumimos que no es null porque esta función solo se llama cuando se puede curar.
// Aquí es donde ocurre la magia de la curación: recorremos cada personaje y aplicamos la lógica de curación
    partida.mi_grupo = partida.mi_grupo.map(personaje => {
        ...
        if (personaje.vida <= 0) {
            personaje.vida = curacion as number; // Si el personaje está muerto, lo revivimos con el 70% de su vida máxima
            // Si el personaje tiene un uso limitado, le recuperamos el uso máximo también
            if (personaje.max_uso) personaje.uso = personaje.max_uso;
            //si está vivo pero perdió vida le recuperamos la vida pero no el uso
        } else {
            personaje.vida += curacion as number; 
...
// Después de curar a todos los personajes, actualizamos el estado de recuperación para evitar abusos
    partida.se_recupera = false as boolean; // Esto es fundamental para que el jugador no pueda curar infinitamente sin pasar por la tienda o la batalla
    ...
    if (btnCurar) btnCurar.disabled = true;

    mostrarGrupoVisual(partida.mi_grupo);
    
    // Modal que muestra que se han curado los personajes
    const modalElement = document.getElementById('curar_informacion') as HTMLElement;
    if (modalElement) {
