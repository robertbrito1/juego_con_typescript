...
export class Combate {
    equipo_aliado:  Personaje[];
    equipo_enemigo: Personaje[];

    // Posiciones actuales (índice del personaje activo en cada equipo)
    turno_aliado:  number;
    turno_enemigo: number;

    // Contadores de bajas
    bajas_aliadas:  number;
    bajas_enemigas: number;

    // Oro acumulado en este combate
    oro_ronda: number;
    oro_total: number;

    // Bandera para saber si el combate ya terminó
    finalizado: boolean;

    // Evita sumar oro/victorias más de una vez si la pantalla se refresca de nuevo
    resultado_guardado: boolean;

    constructor(aliados: Personaje[], enemigos: Personaje[]) {
    ...
    get aliado_actual(): Personaje | null {
        return this.equipo_aliado[this.turno_aliado] ?? null;
    }

    get enemigo_actual(): Personaje | null {
    ...
    private revisar_bajas(): string {
        ...
        // ¿Alguien ganó?
        let victoria: boolean | null = null;
 ...
 */
export function nuevo_combate(aliados: Personaje[], enemigos: Personaje[]): Combate {
 ...
 */
export function atacar(atacante: Personaje, defensor: Personaje): string {
    let daño_base = 0;
    let mensaje = "";

    // ── Cada tipo ataca diferente ──
    if (atacante.tipo === "guerrero") {
        const resultado = (atacante as guerrero).ataque_especial(random(5, 10));
        daño_base = resultado.daño;
        mensaje += resultado.mensaje;

    } else if (atacante.tipo === "mago") {
        const resultado = (atacante as mago).bola_do_fogo();
        daño_base = resultado.daño;
        mensaje += resultado.mensaje;
    ...
    if (defensor.tipo === "ladron") {
        const ladronDef = defensor as ladron;
    ...
    const tiene_ventaja =
        (atacante.tipo === "guerrero" && defensor.tipo === "ladron") ||
 ...
 */
export function escribir_log(mensaje: string): void {
 ...
 */
export function mostrar_duelo(aliado: Personaje | null, enemigo: Personaje | null): void {
 ...
 */
export function actualizar_panel_combate(pelea: Combate): void {
 ...
 */
export function refrescar_pantalla(pelea: Combate): void {
    ...
    if (!aliado) {
        const btnVS = document.getElementById("boton-atacar") as HTMLButtonElement | null;
        if (btnVS) btnVS.disabled = true;

        const aviso = document.getElementById("mensaje-no-aliado");
        if (aviso) aviso.innerText = "No tienes aliados disponibles. Redirigiendo al inicio...";
 ...
 */
function mostrar_resultado_final(pelea: Combate): void {
    ...
    const btnContinuar = modal.querySelector("#btn-modal-continuar") as HTMLElement | null;
    if (btnContinuar) {
        btnContinuar.onclick = () => {
            sessionStorage.removeItem("combateActual");
            sessionStorage.removeItem("resultadoCombate");
            if (comprobacion_de_victoria()) return;
 ...
 */
export function recuperar_combate(): void {
...
function sincronizar_grupo_con_partida(pelea: Combate): void {
    const partida = cargarSesion();
    if (!partida) return;
 ...
 */
function rehidratar_equipo(lista: any[]): Personaje[] {
    if (!lista) return [];
