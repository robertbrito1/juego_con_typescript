...
export function guardarSesion(partida) {
...
export function cargarSesion() {
    ...
    if (partidaJSON) {
        return JSON.parse(partidaJSON);
    }
    return null;
}
export function nombre_aleatorio(lista) {
    const indice = Math.floor(Math.random() * lista.length);
    return lista[indice];
}
export function crearNuevaPartida(modo) {
    const partidaActual = new datos(5000, // dinero
    6, // intento
    [], // grupo_cpu
    [], // mi_grupo
    false, // se_recupera
...
export function cargar_secciones(archivos, tiposeccion) {
    const contenedorPrincipal = document.getElementById("contenedor-juego");
...
export function cargarDatosPantalla() {
    ...
    if (datosRecuperados) {
        ...
        if (document.getElementById("dinero"))
            document.getElementById("dinero").innerText = `💰 Oro: ${partida.dinero}`;
        if (document.getElementById("derrota"))
            document.getElementById("derrota").innerText = `💀 Derrota: ${partida.contador_victoria_cpu} / 2`;
        if (document.getElementById("intentos"))
            document.getElementById("intentos").innerText = `🛒 Intento de compra: ${partida.intento}`;
        if (document.getElementById("informacion"))
            document.getElementById("informacion").innerText = `👥 Miembros: ${partida.mi_grupo.length}`;
...
export function actualizarMarcadoresVisuales(partida) {
    const dineroTexto = document.getElementById("dinero");
    const intentosTexto = document.getElementById("intentos");
    if (dineroTexto)
        dineroTexto.innerText = `💰 Oro: ${partida.dinero}`;
    if (intentosTexto)
        intentosTexto.innerText = `🛒 Intento de compra: ${partida.intento}`;
}
export function comprobacion_de_victoria() {
