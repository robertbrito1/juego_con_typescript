...
import { nombre } from "./personajes.js";
...
export function generarTienda() {
    let indice = 0;
    let cantidad_personajes = 0;
    const personajesTienda = [];
...
export function mostrarTienda() {
    ...
    const promesasCarga = listarPersonajes.map((personaje, indice) => {
        ...
        const promesaAparicion = new Promise((resolve, reject) => {
        ...
        return promesaAparicion
            ...
            col.innerHTML = `
                ...
                `;
            let botoncompra = document.getElementById(`btn-comprar-${indice}`);
            if (botoncompra) {
    ...
    Promise.all(promesasCarga)
        ...
        resultados.forEach((resultado) => {
            if (!resultado || !resultado.botoncompra)
                return;
...
function procesarCompra(personaje) {
